...
def render_recent_news(news):
    news = [n for n in news if n.get("category") != "internship" and not n.get("labOnly")]  # personal site excludes internships + lab-only
    items = []
    for it in news[:10]:
        url = it.get("url") or ""
...
def build_llms(news, team, funding, pubs):
    ...
    for n in [x for x in news if x.get("category") != "internship" and not x.get("labOnly")][:10]:
